...
static void init_ttables() {
    for (int i = 0; i < 256; i++) {
        uint8_t x = SBOX[i];
        uint32_t v = ((uint32_t)x << 24) | ((uint32_t)x << 16) | ((uint32_t)x << 8) | x;
        uint32_t t = L(v);
        T0[i] = (t & 0xFF000000) |
            ((t & 0x00FF0000) >> 8) |
            ((t & 0x0000FF00) >> 16) |
...
// T函数优化版
static inline uint32_t T_opt(uint32_t x) {
    return T0[(x >> 24) & 0xFF] ^
        T1[(x >> 16) & 0xFF] ^
        T2[(x >> 8) & 0xFF] ^
        T3[x & 0xFF];
...
static void sm4_key_schedule(const uint8_t* key, uint32_t rk[32]) {
    uint32_t MK[4];
    for (int i = 0; i < 4; i++) {
