...
// 使用VPROLD实现循环左移
static inline __m512i rol32(__m512i x, int n) {
    return _mm512_rol_epi32(x, n);
}

// L变换
static inline __m512i L_avx512(__m512i b) {
    return _mm512_xor_si512(
        _mm512_xor_si512(
            _mm512_xor_si512(
                _mm512_xor_si512(b, rol32(b, 2)),
                rol32(b, 10)),
            rol32(b, 18)),
        rol32(b, 24));
}
...
// S-box替换（使用vpermb）
static __m512i sm4_sbox_avx512(__m512i x) {
    // AVX512BW支持vpermb
    __m512i lut = _mm512_loadu_si512(SBOX);
    return _mm512_permutexvar_epi8(x, lut);
}

// T函数
static inline __m512i T_avx512(__m512i x) {
    return L_avx512(sm4_sbox_avx512(x));
}
...
void sm4_encrypt8_avx512(const uint8_t in[8][16], uint8_t out[8][16], const uint32_t rk[32]) {
    ...
    // 存储逆序
    _mm512_store_si512((__m512i*)tmp, X3);
    for (int i = 0; i < 8; i++) memcpy(out[i], &tmp[i * 4], 16);
}
